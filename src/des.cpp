...
// --- Konstanta dan Tabel Standar DES (diambil dari Tugas 1) ---
...
std::string des_encrypt(const std::string& plaintext, const std::string& key) {
    if (key.length() != 8) {
        throw std::invalid_argument("Kunci harus tepat 8 karakter.");
    }
    ...
    // Process data block by block (8 bytes / 64 bits per block)
